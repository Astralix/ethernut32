...
#define HSI_VALUE  ((uint32_t)16000000) /*!< Value of the Internal High Speed oscillator in Hz.
 ...
 /**
  * @brief In the following line adjust the Internal Medium Speed oscillator (MSI) Startup
    Timeout value
    */
 #if !defined  (MSI_STARTUP_TIMEOUT)
 #define MSI_STARTUP_TIMEOUT   ((uint16_t)0x0500) /*!< Time out for HSI start up */
 #endif

 #if !defined  (MSI_VALUE)
 #define MSI_VALUE  ((uint32_t)65536) /*!< Value of the Internal Medium Speed oscillator in Hz.
                                              The real value may vary depending on the variations
                                              in voltage and temperature.  */
 #endif
